...
def debug_basak_commercial_biomass():
    ...
    survey_fish_data_df = calculate_biomass(
        survey_fish_data_df, "data/constants/biomass_coeffs_fish.csv"
    )
    print(f"   Records with biomass: {len(survey_fish_data_df)}")

    # Filter for Basak Can-Unsang MPA
    print("\n5. Filtering for Basak Can-Unsang MPA...")
    basak_data = survey_fish_data_df[
        survey_fish_data_df["Site"] == "Basak Can-Unsang MPA"
    ]
    ...
    commercial_basak_2025 = basak_2025[
        basak_2025["Species"].isin(commercial_fish_names)
    ]
    ...
    if len(commercial_basak_2025) > 0:
        ...
        commercial_biomass_by_period = (
            commercial_basak_2025.groupby("Period")["Total Biomass"].sum().reset_index()
        )
        commercial_biomass_by_period["Commercial Biomass (kg)"] = (
            commercial_biomass_by_period["Total Biomass"] / 1000
        )
        ...
        for _, row in commercial_biomass_by_period.iterrows():
            ...
            if (period, "Basak Can-Unsang MPA") in dive_numbers_df.index:
                ...
                print(
                    f"     {period}: {commercial_biomass_kg:.2f} kg ÷ {dives} dives = {density:.2f}"
                )
        ...
        for data_species in all_species:
            for comm_species in commercial_species:
                if (
                    data_species.lower() in comm_species.lower()
                    or comm_species.lower() in data_species.lower()
                ):
