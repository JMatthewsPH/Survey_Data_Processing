...
def calculate_subs_metrics(pre_processed_subs_data_df, daily_dive_numbers_df, period):
    ...
    # Each subfunction now returns daily rows per metric
    daily_frames = []
    daily_frames.append(calculate_hard_coral_cover(daily_subs_data_df, None, daily_dive_numbers_df))
    daily_frames.append(calculate_soft_coral_cover(daily_subs_data_df, None, daily_dive_numbers_df))
    daily_frames.append(calculate_fresh_algae_cover(daily_subs_data_df, None, daily_dive_numbers_df))
    daily_frames.append(calculate_rubble_cover(daily_subs_data_df, None, daily_dive_numbers_df))
    daily_frames.append(calculate_bleaching(daily_subs_data_df, None, daily_dive_numbers_df))

    # Merge all daily metrics together on Date, Period, and Site
    from functools import reduce
    merged_daily = reduce(
        lambda left, right: pd.merge(left, right, on=["Date", "Period", "Site"], how="outer"),
        daily_frames
    ).fillna(0)

    # Summarize across days (mean, SD, SE, CI, EB)
    coverage_columns = [
        "Hard Coral Cover", "Soft Coral Cover", "Fresh Algae Cover",
        "Rubble Cover", "Bleaching"
    ]
    value_cols = [c for c in coverage_columns if c in merged_daily.columns]
...
def calculate_hard_coral_cover(daily_subs_data_df, results_df, daily_dive_numbers_df):
    ...
    return hard_coral_daily[["Date", "Period", "Site", "Hard Coral Cover"]]
...
def calculate_soft_coral_cover(daily_subs_data_df, results_df, daily_dive_numbers_df):
    ...
    return soft_coral_daily[["Date", "Period", "Site", "Soft Coral Cover"]]
...
def calculate_fresh_algae_cover(daily_subs_data_df, results_df, daily_dive_numbers_df):
    ...
    return fresh_algae_daily[["Date", "Period", "Site", "Fresh Algae Cover"]]
...
def calculate_rubble_cover(daily_subs_data_df, results_df, daily_dive_numbers_df):
    rubble_daily = (
        daily_subs_data_df[daily_subs_data_df["Group"].str.contains("Rubble")]
        .groupby(["Date", "Period", "Site"])["Total"]
        .sum()
   ...
   rubble_daily["Rubble Cover"] = rubble_daily.apply(
    ...
    return rubble_daily[["Date", "Period", "Site", "Rubble Cover"]]
...
def calculate_bleaching(daily_subs_data_df, results_df, daily_dive_numbers_df):
    bleaching_categories = ["Fully Bleaching", "Partially Bleaching"]
    bleaching_daily = (
        daily_subs_data_df[daily_subs_data_df["Group"].isin(bleaching_categories)]
        .groupby(["Date", "Period", "Site"])["Total"]
        .sum()
        .reset_index()
    )
    ...
    return _daily[["Date", "Period", "Site", "Bleaching"]]
